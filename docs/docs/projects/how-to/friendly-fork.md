...
Liquid V3 is published under a Business Source License (BUSL) such that while the codebase is source available, any commercial deployment needs to be approved by the Liquid Association.
...
- You will receive a BUSL license for 3 years, enabling the team to use Liquid code for non-profit use. You cannot deploy the code on any chain where Liquid V3 is already live.
- Direct support from the Liquid core team.
- Marketing support from Liquid social channels.
...
- Each friendly fork needs to go through the Liquid association and obtain a BUSL license.
- Each friendly fork is required to audit any changes made to the Liquid V3 codebase in their specific deployment.
