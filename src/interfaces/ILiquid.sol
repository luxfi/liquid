...
struct LiquidInitializationParams {
...
/// @notice This redemption struct is included in the main contract, Liquid.sol, to aid in calculating user debt from historic redemptions.
...
interface ILiquidActions {
    ...
    ///
    /// @notice **_NOTE:_** When depositing, the `Liquid` contract must have **allowance()** to spend funds on behalf of **msg.sender** for at least **amount** of the **yieldToken** being deposited.  This can be done via the standard `ERC20.approve()` method.
    ///
    /// @notice **Example:**
    /// @notice ```
    /// @notice address ydai = 0xdA816459F1AB5631232FE5e97a05BBBb94970c95;
    /// @notice uint256 amount = 50000;
    /// @notice IERC20(ydai).approve(liquidAddress, amount);
    /// @notice Liquid(liquidAddress).deposit(amount, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice ```
    /// @notice address ydai = 0xdA816459F1AB5631232FE5e97a05BBBb94970c95;
    /// @notice (uint256 LTV, ) = Liquid(liquidAddress).getLoanTerms(msg.sender);
    /// @notice (uint256 yieldTokens, ) = Liquid(liquidAddress).getCDP(tokenId);
    /// @notice uint256 maxWithdrawableTokens = (Liquid(liquidAddress).LTV() - LTV) * yieldTokens / LTV;
    /// @notice Liquid(liquidAddress).withdraw(maxWithdrawableTokens, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice Liquid(liquidAddress).mint(amtDebt, msg.sender);
    /// @notice ```
    ///
    /// @param tokenId The tokenId of account.
    ...
    /// @notice ```
    /// @notice uint256 amtDebt = 5000;
    /// @notice Liquid(liquidAddress).mintFrom(msg.sender, amtDebt, msg.sender);
    /// @notice ```
    ///
    /// @param tokenId   The tokenId of account.
    /// @param amount    The amount of tokens to mint.
    ...
    /// @notice Liquid(liquidAddress).burn(amtBurn, 420);
    /// @notice ```
    ///
    ...
    /// @notice Liquid(liquidAddress).repay(amtRepay, msg.sender);
    /// @notice ```
    ///
     ...
     * @notice **Example:**
     * @notice ```
     * @notice LiquidV2(liquidAddress).liquidate(id4);
     * @notice ```
     *
    ...
    /// @notice **Example:**
    /// @notice ```
    /// @notice Liquid(liquidAddress).batchLiquidate([id1, id35]);
    /// @notice ```
    ///
    ...
    /// @notice Redeems `amount` debt from the liquid in exchange for yield tokens sent to the transmuter.
    ///
    ...
    ///
    /// @notice This function is only callable by the owner of the token id or the LiquidPosition contract.
    ///
...
interface ILiquidAdminActions {
    ...
    /// @notice Set a new liquid deposit cap.
    ///
    /// @notice `msg.sender` must be the admin or this call will revert with an {Unauthorized} error.
    ///
    ...
    /// @notice Pause all future deposits in the Liquid.
    ///
    ...
    ///
    /// @param isPaused The new pause state for deposits in the liquid.
    function pauseDeposits(bool isPaused) external;

    /// @notice Pause all future loans in the Liquid.
    ///
    /// @notice `msg.sender` must be the admin or guardian or this call will revert with an {Unauthorized} error.
    ///
    /// @notice Emits a {LoansPaused} event.
    ///
    /// @param isPaused The new pause state for loans in the liquid.
    function pauseLoans(bool isPaused) external;

    /// @notice Set the liquid Fee vault.
    ///
    /// @notice `msg.sender` must be the admin or this call will revert with an {Unauthorized} error.
    ///
    /// @notice Emits a {LiquidFeeVaultUpdated} event.
    ///
    /// @param value The address of the new liquid Fee vault.
    function setLiquidFeeVault(address value) external;
}

interface ILiquidEvents {
    /// @notice Emitted when the pending admin is updated.
    ///
    /// @param pendingAdmin The address of the pending admin.
    event PendingAdminUpdated(address pendingAdmin);

    /// @notice Emitted when the liquid Fee vault is updated.
    ///
    /// @param liquidFeeVault The address of the liquid Fee vault.
    event LiquidFeeVaultUpdated(address liquidFeeVault);
    ...
    /// @notice Emitted when a guardian is added or removed from the liquid.
    ///
    ...
    /// @notice Emitted when a new token adapter is set in the liquid.
    ///
    ...
    /// @notice Emitted when deposits are paused or unpaused in the liquid.
    ///
    /// @param isPaused The current pause state of deposits in the liquid.
    event DepositsPaused(bool isPaused);

    /// @notice Emitted when loans are paused or unpaused in the liquid.
    ///
    /// @param isPaused The current pause state of loans in the liquid.
...
interface ILiquidImmutables {
    /// @notice Returns the version of the liquid.
    ///
    /// @return The version.
...
interface ILiquidState {
    ...
    function liquidPositionNFT() external view returns (address nftContract);
    ...
    function liquidFeeVault() external view returns (address vault);
    ...
    ///
    /// @notice Collateralization is determined by taking the total value of collateral deposited in the liquid and dividing it by the total debt.
    ///
    /// @dev The value returned is a 18 decimal fixed point integer.
    ///
    ...
    /// @param liquidCurrentCollateralization Current collateralization ratio of the liquid
    /// @param liquidMinimumCollateralization Minimum collateralization ratio of the liquid to trigger full liquidation
    ...
    /// @dev Gets total value deposited in the liquid
    ///
    ...
    /// @dev Gets total underlying value locked in the liquid.
    ///
...
interface ILiquidErrors {
    ...
    error LiquidNFTZeroAddressError();

    /// @notice An error which is used to indicate that the NFT address for the Liquid has already been set
    error LiquidNFTAlreadySetError();

    /// @notice An error which is used to indicate that the token address for the LiquidTokenVault does not match the underlyingToken
    error LiquidVaultTokenMismatchError();
...
/// @title  ILiquid
/// @author Lux Liquid
interface ILiquid is ILiquidActions, ILiquidAdminActions, ILiquidErrors, ILiquidImmutables, ILiquidEvents, ILiquidState {}
