...
import "../interfaces/ILiquidTransmuter.sol";
...
contract TransmuterTest is Test {
    ...
    function setUp() public {
        ...
        transmuter = new LiquidTransmuter(ILiquidTransmuter.TransmuterInitializationParams(address(alETH), address(this), 5_256_000, 0, 0, 52_560_000 / 2));

        transmuter.setLiquid(address(liquid));

        transmuter.setDepositCap(uint256(type(int256).max));

        deal(address(collateralToken), address(liquid), type(uint256).max);
    ...
    function testURI() public {
        ...
        LiquidTransmuter.StakingPosition memory position = transmuter.getPosition(1);
    ...
    function testCreateRedemptionDepositCapReachedSynthetic() public {
        ...
        liquid.setSyntheticsIssued(90e18);
    ...
    function testClaimRedemption() public {
        ...
        assertEq(collateralToken.balanceOf(address(0xbeef)), liquid.convertUnderlyingTokensToYield(100e18));
    ...
    function testClaimRedemptionBadDebt() public {
        ...
        liquid.setSyntheticsIssued(1200e18);
        vm.prank(address(0xbeef));
        transmuter.createRedemption(100e18);
        ...
        liquid.setUnderlyingValue(200e18);

        vm.prank(address(0xbeef));
        transmuter.claimRedemption(1);

        assertEq(collateralToken.balanceOf(address(0xbeef)), liquid.convertUnderlyingTokensToYield(100e18) / 2);
        assertEq(alETH.balanceOf(address(transmuter)), 0);
    }
    ...
    function testClaimRedemptionFromLiquid() public {
        vm.prank(address(0xbeef));
        transmuter.createRedemption(100e18);
        ...
        uint256 startingBalance = collateralToken.balanceOf(address(liquid));
        ...
        assertEq(collateralToken.balanceOf(address(liquid)), startingBalance - liquid.convertUnderlyingTokensToYield(100e18));
    }
    ...
    function testFuzzClaimRedemption(uint256 amount) public {
        ...
        assertEq(collateralToken.balanceOf(address(0xbeef)), liquid.convertUnderlyingTokensToYield(amount));
        assertEq(alETH.balanceOf(address(transmuter)), 0);
    }
    ...
    function testClaimRedemptionWithTransmuterFee() public {
        ...
        assertEq(collateralToken.balanceOf(address(0xbeef)), liquid.convertUnderlyingTokensToYield(100e18) * 9900 / 10_000);
        assertEq(collateralToken.balanceOf(address(this)), liquid.convertUnderlyingTokensToYield(100e18) * 100 / 10_000);
        assertEq(alETH.balanceOf(address(transmuter)), 0);
    }
    ...
    function testFuzzClaimRedemptionWithTransmuterFee(uint256 fee) public {
        ...
        assertEq(collateralToken.balanceOf(address(0xbeef)), liquid.convertUnderlyingTokensToYield(100e18) * (10_000 - fee) / 10_000);
        assertEq(collateralToken.balanceOf(address(this)), liquid.convertUnderlyingTokensToYield(100e18) * fee / 10_000);
        assertEq(alETH.balanceOf(address(transmuter)), 0);
    }
    ...
    function testClaimRedemptionPremature() public {
        ...
        assertEq(collateralToken.balanceOf(address(0xbeef)), liquid.convertUnderlyingTokensToYield(50e18));
    ...
    function testFuzzClaimRedemptionPremature(uint256 time) public {
        ...
        assertApproxEqAbs(collateralToken.balanceOf(address(0xbeef)), liquid.convertUnderlyingTokensToYield((100e18 * time) / 5_256_000), 1);
