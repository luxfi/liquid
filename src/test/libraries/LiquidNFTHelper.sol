...
library LiquidNFTHelper {
     ...
     * @param nft The address of the LiquidPosition NFT contract.
    ...
    function getAllTokenIdsForOwner(address owner, address nft) public view returns (uint256[] memory tokenIds) {
        ...
        uint256 tokenCount = ILiquidPosition(nft).balanceOf(owner);
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            tokenIds[i] = ILiquidPosition(nft).tokenOfOwnerByIndex(owner, i);
        }
    }
