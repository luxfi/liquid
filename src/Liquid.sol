...
import {ILiquidETHVault} from "./interfaces/ILiquidETHVault.sol";
...
contract Liquid is ILiquid, Initializable {
    ...
    /// @inheritdoc ILiquidState
    address public liquidPositionNFT;
    ...
    event LiquidPositionNFTMinted(address indexed to, uint256 indexed tokenId);

    /// @notice Sets the NFT position token, callable by admin.
    function setLiquidPositionNFT(address nft) external onlyAdmin {
        if (nft == address(0)) {
            revert LiquidNFTZeroAddressError();
        }

        if (liquidPositionNFT != address(0)) {
            revert LiquidNFTAlreadySetError();
        }

        liquidPositionNFT = nft;
    }

    /// @inheritdoc ILiquidAdminActions
    function setLiquidFeeVault(address value) external onlyAdmin {
        if (IFeeVault(value).token() != underlyingToken) {
            revert LiquidVaultTokenMismatchError();
        }
        liquidFeeVault = value;
        emit LiquidFeeVaultUpdated(value);
    }
    ...
    function deposit(uint256 amount, address recipient, uint256 tokenId) external returns (uint256) {
        ...
        if (tokenId == 0) {
            tokenId = ILiquidPosition(liquidPositionNFT).mint(recipient);
            emit LiquidPositionNFTMinted(recipient, tokenId);
        } else {
    ...
    function burn(uint256 amount, uint256 recipientId) external returns (uint256) {
        ...
        if (credit > totalSyntheticsIssued - ILiquidTransmuter(transmuter).totalLocked()) {
            revert BurnLimitExceeded(credit, totalSyntheticsIssued - ILiquidTransmuter(transmuter).totalLocked());
        }
        ...
        returns (uint256 totalAmountLiquidated, uint256 totalFeesInYield, uint256 totalFeesInUnderlying)
    {
        ...
        for (uint256 i = 0; i < accountIds.length; i++) {
            ...
            if (accountId == 0 || !_tokenExists(liquidPositionNFT, accountId)) {
                continue;
            }
    ...
    function resetMintAllowances(uint256 tokenId) external {
        ...
        if (msg.sender != address(liquidPositionNFT)) {
            // Direct call - verify caller is current owner
            address tokenOwner = IERC721(liquidPositionNFT).ownerOf(tokenId);
        ...
        returns (uint256 amountLiquidated, uint256 feeInYield, uint256 feeInUnderlying)
    {
        ...
        if (outsourcedFee > 0) {
            ...
            if (vaultBalance > 0) {
                ...
                IFeeVault(liquidFeeVault).withdraw(msg.sender, feeInUnderlying);
            }
        }
    ...
    function _checkForValidAccountId(uint256 tokenId) internal view {
        if (!_tokenExists(liquidPositionNFT, tokenId)) {
    ...
    /// @dev Calculates the total value of the liquid in the underlying token.
    /// @return totalUnderlyingValue The total value of the liquid in the underlying token.
