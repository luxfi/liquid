...
contract LiquidStrategy is ILiquidStrategy, Ownable {
    ...
    event LiquidLog(string message, uint256 value);

    /// @notice Modifier to restrict access to the vault **managed** by the VAULT contract
    modifier onlyVault() {
        require(msg.sender == address(VAULT), "Only vault can call this function");
        _;
    }

    constructor(address _vault, StrategyParams memory _params) Ownable(_params.owner) {
    ...
    {
        uint256 oldAllocation = abi.decode(data, (uint256));
        emit LiquidLog("oldAllocation", oldAllocation);
        uint256 amountDeallocated = _deallocate(assets);
        emit LiquidLog("amountDeallocated", amountDeallocated);

        uint256 newAllocation = oldAllocation - amountDeallocated;
        emit LiquidLog("newAllocation", newAllocation);
    ...
    function deallocateDex(bytes calldata quote, bool prevSettler) external returns (uint256 ret) {
        ...
        require(ZeroXSwapVerifier.verifySwapCalldata(quote, address(this), address(VAULT.asset()), limit));
